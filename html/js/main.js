...
// 3. 신간 도서 영역 스크립트 (클릭 시 sub.html 이동)
{
  ...
  function renderNewBooks(books) {
    ...
    remainingBooks.forEach(book => {
      ...
      card.innerHTML = `
        ...
        <div class="sub-book-author" title="${authors}">${authors}</div>
      `;
  ...
  function handleSearch() {
    ...
    if (query) {
      fetchNewBooks(query);
      if (window.filterRanking) window.filterRanking(query);
    }
  }
...
// 4. 출판사 추천 도서 연동 스크립트 (클릭 시 sub.html 이동)
{
...
// 5. AI 추천 큐레이션 스크립트 (클릭 시 sub.html 이동)
{
  const KAKAO_REST_API_KEY = '9a318e0b10ce20c64ed53f01aa34e8c1';
        ...
        if (res.ok) {
          ...
          if (data.documents && data.documents.length > 0) {
            ...
            card.innerHTML = `
              ...
              <a href="sub.html" class="btn-ai-read">
